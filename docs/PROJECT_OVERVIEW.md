...
Issue #2 Phase 3Aでは完成外装にbaseline／D2a／D2c3をquery限定で重ね、198枚の実WebGL PNGと性能・回帰を保存した。D2a／D2c3は矩形影を除去するが、前後輝度差、A.5ライト契約、性能差分を満たさない。状態は`ISSUE2_FINAL_POLISH_PHASE3A_COMPARISON_ONLY_NOT_ADOPTED`で、通常pathとPhase 3C.1／3C.2／3C.3-onlyを変更せず、採用候補なしとする。
