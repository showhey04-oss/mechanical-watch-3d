...
現在の改修フェーズは「最終外装統合 — Phase 3C.2 正式黒革ストラップ・尾錠候補」とする。Phase 3C.1 Head `4de3c018f52ea88d1cbe5f4ad0c44166f7f89914`は`HUMAN_ACCEPTED_PHASE3C1_WITH_DEFERRED_QUALITY_ITEMS`であり、Phase 3C.2はそのHeadへ積み上げる別Draftである。v3.15.0はS86を通常寸法として採用し、v3.14.0の内部機構、レンダリング、照明、影、構造透過、DPR、カメラを維持している。継承した小秒文字板の選択性、矩形影、transparent／depthWrite不連続、透過時の暗部・深度順、PC／iPhone照明差は、PR #5のD2c3を取り込まずIssue #2または後続選択改善へ分離する。
