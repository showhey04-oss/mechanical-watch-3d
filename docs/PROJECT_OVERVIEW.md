...
現在の改修フェーズは「最終外装統合 — Phase 3A 外装インターフェース監査・候補設計」とする。v3.15.0はS86を通常寸法として採用し、v3.14.0の内部機構、レンダリング、照明、影、材質、構造透過、DPR、カメラを維持している。継承した既定照明が実機で暗く見えることは既知事項として、PR #5のD2c3を取り込まずIssue #2の照明最終調整へ申し送る。
...
寸法・比率調整はPR #10のS86採用とPR #11のPhase 2C監査で完了扱いとする。Phase 3Aは通常Sceneへケース、ベゼル、風防、物理文字板、裏蓋、ラグ、ストラップを追加せず、現行モデルと外装の接続条件、正面開口、前後クリアランス、りゅうず・巻真軸、装着部の候補値を比較する監査工程である。E-BALANCEDを`RECOMMENDED_NOT_ADOPTED`として提示するが、人間承認前に通常表示へ採用しない。ETA 4.50 mmは`REFERENCE_DATUM_UNRESOLVED / UNVERIFIED`のまま外装厚さの調整量に使用せず、実装時に表示開口と全体比率を再確認する。
