...
Refactor Aで輪列・高さ面・回転状態をパラメータ化し、PR #3／#4でUIを整理した。v3.14.0では機構と描画の既存挙動を変更せず、既存のビート番号・実Object3D回転角・ユーザーのりゅうず切替完了を読み取る機構同期作動音を追加する。
...
内部機構、レンダリング、照明、影、材質、構造透過、DPR、カメラはv3.13.0の基準を維持する。モバイル実機の暗部・影・露出差は引き続きIssue #2の独立スコープとする。
