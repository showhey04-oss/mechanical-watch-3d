...
現在の改修フェーズは「最終外装統合 — Phase 3C.1 正式時計本体意匠候補」とする。Phase 3C.1は人間承認済みPhase 3B.2 Head `98d83781aa7aa001836a0d57f1ad6e3d058a15c4`へ積み上げる別Draftであり、既定採用ではない。v3.15.0はS86を通常寸法として採用し、v3.14.0の内部機構、レンダリング、照明、影、材質、構造透過、DPR、カメラを維持している。継承した既定照明が実機で暗く見えることは既知事項として、PR #5のD2c3を取り込まずIssue #2の照明最終調整へ申し送る。
...
Phase 3C.1では、Phase 3B.2構造を保護したまま、query時だけ正式時計本体意匠を生成する。実テンプworld中心[7.700,1.730,1.800]を文字板へ投影し、直径6.600、縁0.320の限定オープンハートを配置した。文字板と地板の遮蔽を物理holeで除き、中央軸受、受、支持、石、ねじ座を保持する。参照画像の名目的な9時位置を模写せず、実モデルの約2時34分方向を優先する。ポリッシュケース、ドーム風防、アイボリー文字板、バーindex、細身3針、6時小秒を実装するが、機構・照明・影・UI・音響は変更しない。PC／物理iPhone人間確認前に採用しない。黒革、実用長、巻込み、穴列、定革・遊革、尾錠枠・つく棒・取付バーはPhase 3C.2の必須工程とする。
