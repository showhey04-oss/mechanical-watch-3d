...
Phase 3C.3では、`integration=phase3c3`追加時だけ小秒凹面へ非描画selection proxyを4枚生成し、既存の小秒表示へ選択を委譲する。Desktop／390×844の100%／50%で4/4、16%時の内部選択、外装ON/OFF、split／explode、復元、位置1／2、三針拘束、Object3D整合を確認した。queryなし、Phase 3C.1-only、Phase 3C.2-onlyは承認Headとpixel exactである。Issue #2の矩形影、透過不連続、前後輝度差、PC／iPhone差、D2c3は修正せず引き継ぐ。PC／物理iPhone人間確認前にReady化・マージ・既定採用を行わない。
