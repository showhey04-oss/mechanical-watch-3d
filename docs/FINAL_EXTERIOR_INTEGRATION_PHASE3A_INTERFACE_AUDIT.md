...
3候補はいずれも必須包含条件を満たす。Codexの推奨はE-BALANCEDだが、状態は`RECOMMENDED_NOT_ADOPTED`である。人間承認までは候補比較履歴として扱い、通常表示へ採用しない。
...
| crown outer projection at position 2 | 2.225 | 1.925 | 1.625 |
...
ケースチューブ軸は現行巻真軸Y=-1.050、Z=-4.500へ一致させる。りゅうず中心Xは位置1で19.800、位置2で21.150であり、1.350のA.7絶対移動を維持する。候補ケース外径から求める位置1の外部突出量はE-COMPACT 0.875、E-BALANCED 0.575、E-EDUCATIONAL 0.275、位置2最大突出量は順に2.225、1.925、1.625である。

ケース開口、チューブ外径、局所リセス、指掛かりは実外装Geometryで確認する。機構側の軸や移動量を外装へ合わせて変更しない。
...
- 長所：外径・全厚・描画面積が最小
- 短所：前後・半径方向の余裕が最小で、局所干渉と回帰リスクが高い

### E-BALANCED

- 長所：S86の余白、機構観察、外径、ラグ、前後余裕の均衡がよい
- 短所：ケースチューブ、裏蓋開口、透明外装は実装時の確認が必要
...
推奨はE-BALANCED、状態は`RECOMMENDED_NOT_ADOPTED`とする。E-COMPACTより実装余裕があり、E-EDUCATIONALより外径、全厚、透明面積、描画・回帰リスクを抑えられるためである。
...
候補輪郭は保存PNGへの後処理overlayであり、通常SceneへObject3Dを追加していない。証跡一覧、SHA-256、生成元は`docs/evidence/final-exterior-interface-phase3a/README.md`と`evidence-manifest.json`を参照する。
