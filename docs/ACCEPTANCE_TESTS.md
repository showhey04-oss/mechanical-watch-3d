...
### Phase 3B.1 E-BALANCED第2外装比率候補

- `?exterior=balanced`限定でケース胴外径が前端39.000、中央最大39.600、後端39.000となり、Y=-2.860～4.635の指定6点プロファイルを通る
- ケース胴軸方向厚7.495、前後外装突出各0.600、外装総厚8.695を区別し、`0.600 + 7.495 + 0.600 = 8.695`を満たす
- 風防と裏蓋リングの軸方向厚が各0.600で、針前面余裕0.350、ブリッジ後面余裕0.400を維持する
- ベゼルが表示開口29.800、風防有効径30.600、背面外径38.800、正面外径37.600の単一閉合テーパープロファイルで、外周へ向かって軸方向に厚くならない
...
- 必要逃げ0.298836、採用逃げ0.310872、上限差0.019128、位置1実gap 0.030088、位置2実gap 1.380088、最小壁厚0.589128を実Geometryから再測定する
...
- 透過100%／16%と内部選択の人間確認合格を継承し、透過50%は第2候補の人間確認で判断する
