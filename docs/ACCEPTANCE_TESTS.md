...
- 上記は自動viewportの幾何条件として維持する。物理iPhoneだけに残る入力欄右端の軽微な表示ずれは機能合格と分離し、最終微調整工程で再確認する
...
- Phase 2A.2の候補比較は完了とし、追加照明反復と下記の物理iPhone採用判定は、[最終微調整申し送り](ISSUE2_FINAL_POLISH_HANDOFF.md)の依存工程が完了するまで延期する。実施時期だけを延期し、試験条件と合格基準は緩和しない
