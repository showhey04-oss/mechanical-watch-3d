...
### Phase 3B.1 E-BALANCEDケース胴シルエット候補

- `?exterior=balanced`限定でケース胴外径が前端39.000、中央最大39.600、後端39.000となり、Y=-3.060～4.885の指定6点プロファイルを通る
- ケース胴軸方向厚7.945、前後外装突出各0.950、外装総厚9.845を区別し、`0.950 + 7.945 + 0.950 = 9.845`を満たす
- 内周半径18.900を全対象頂点で維持し、局所逃げ後の最小壁厚が0.550以上である
- りゅうず実Meshのコア・外周歯包絡から必要逃げを算出し、固定0.330ではなくgap 0.030を満たす必要最小値を適用する
- 必要逃げ0.298836、採用逃げ0.309461、上限差0.020539、位置1実gap 0.030084、位置2実gap 1.380084、最小壁厚0.590539を実Geometryから再測定する
- 旧0.150では物理食い込み0.121192、目標gap込み不足0.151192が残ることを証跡へ保存する
...
- 位置1のチューブ外端へのraw gap -0.056857を`PHASE3B1_IMPLEMENTATION_ASSUMPTION`として維持し、指掛かり・pull／push操作性は`UNVERIFIED`とする
