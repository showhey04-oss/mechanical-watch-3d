...
- `?exterior=balanced`限定の第3候補として、総厚8.695を維持したままケース胴端径を38.900へ絞り、指定6点プロファイルで前後テーパーだけを増強。
- りゅうずの実Mesh包絡から位置1の必要最小逃げ0.298836を算出し、新プロファイルで実採用値0.319108を適用。上限0.330との差0.010892、位置1gap 0.030026、最小壁厚0.550000を確認。
...
- ベゼルの表示開口29.800、背面外径38.800を維持し、正面外径37.000、内周正面Y=-3.240、外縁正面Y=-2.880へテーパーを増強。裏蓋リングは厚さ0.600と前面外径39.000を維持し、後面外径を37.800へ絞った。
- ムーブメントとケース内周の空隙へ、外径37.650、内径36.750、Y=4.035～4.485の閉合保持リングを追加。pick priority -1、構造透過対象、禁止干渉0、固定方式／製造公差／防水は`UNVERIFIED`。
- 外装総厚8.695の方向性、りゅうず位置1／2、指掛かり、pull／push、保持リング方針、透過16%、回転・ズーム、時計機能、作動音、物理iPhone操作性は合格済み。第3候補のケース胴・ベゼル・裏蓋リングのテーパーと透過50%は人間確認待ち。
- Node 109/109、desktop機能84/84、390×844総合88/88、外装診断、S86、Phase 2C、三針拘束、禁止干渉に合格。10秒idle候補差分はfps -0.19%、p95 +0.00msで差分基準内。デスクトップA.6絶対性能2項目はin-app Browser環境制約で未達、閾値は変更していない。
- 通常path、S86、Phase 2C、A.7、機構、カメラ、照明、材質、UI、作動音、APP_VERSIONは変更しない。状態は`IMPLEMENTATION_CANDIDATE_NOT_DEFAULT`のまま、第3候補のPC／物理iPhone確認前に既定採用しない。
