...
- `?exterior=balanced`限定の第2候補として、ケース胴Y範囲を-2.860～4.635、ケース胴軸方向厚を7.495、前後外装突出を各0.600、外装総厚を8.695へ縮小。外径39.000→39.600→39.000の単一閉合プロファイルを維持。
- りゅうずの実Mesh包絡から位置1の必要最小逃げ0.298836を算出し、新プロファイルで実採用値0.310872を適用。上限0.330との差0.019128、位置1gap 0.030088、最小壁厚0.589128を確認。
...
- ベゼルを表示開口29.800、背面外径38.800、正面外径37.600の単一閉合テーパープロファイルへ変更し、風防／裏蓋リング厚を各0.600へ縮小。
- ムーブメントとケース内周の空隙へ、外径37.650、内径36.750、Y=4.035～4.485の閉合保持リングを追加。pick priority -1、構造透過対象、禁止干渉0、固定方式／製造公差／防水は`UNVERIFIED`。
- 第1候補の人間確認でりゅうず位置1、指掛かり、pull／push、位置2、カメラ、選択、透過100%／16%、時計機能、作動音、PC／物理iPhone性能は合格済み。透過50%と第2候補の外装比率は人間確認待ち。
- Node、外装診断、S86、Phase 2C、三針拘束、禁止干渉、390×844 UI 22/22、HUD 57/57は合格。in-app Browserでは通常pathと候補の双方が絶対フレーム閾値未達だが、候補差分はfps -2.81%、p95 -0.30msで差分基準内。閾値は変更していない。
- 通常path、S86、Phase 2C、A.7、機構、カメラ、照明、材質、UI、作動音、APP_VERSIONは変更しない。状態は`IMPLEMENTATION_CANDIDATE_NOT_DEFAULT`のまま、第2候補のPC／物理iPhone確認前に既定採用しない。
