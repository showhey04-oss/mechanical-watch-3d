...
Phase 3C.3で完成した時計全体に対して、現行baseline、PR #5のD2a、D2c3をquery限定で再構成して比較した。D2a／D2c3は矩形影を除去し暗部を持ち上げるが、既存A.5ライト契約、前後輝度差、同一環境の性能差分を満たさない。いずれも通常表示へ採用せず、PR #5、Issue #2、D2c3の保留状態を維持する。
...
| issue2-d2c3 | PR #5 D2c3の中間調再配分、2灯＋lower bounce | `RETAIN_AS_VISUAL_REFERENCE_REJECT_FOR_ADOPTION` |
...
Phase 3Aの3候補から採用候補は選ばない。D2a／D2c3は視覚比較履歴として残す。Issue #2を解決する次候補は、完成外装baselineの前後均衡とA.5契約を保ち、RectAreaLightによる性能悪化を避ける別設計として新たに評価する必要がある。物理iPhone評価と15分発熱再試験は、技術ゲートを満たす新候補ができるまで実施しない。
