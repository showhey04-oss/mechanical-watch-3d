...
const candidate =
  params.get("candidate") || "issue2-phase3b1c-baseline";
const matrix = params.get("matrix") || "single";
const upload = params.get("upload") === "1";
const frame = document.getElementById("appFrame");
const output = document.getElementById("issue2Phase3B1cResult");
...
const waitFor = async (test, timeoutMs = 60_000) => {
  ...
  throw new Error("timed out waiting for Phase 3B.1c diagnostics");
};
...
async function analyzePng(blob) {
  ...
  for (let y = 0; y < canvas.height; y += stride) {
    for (let x = 0; x < canvas.width; x += stride) {
      ...
      modelLuminance.push(value);
      if (value < 0.045) darkCount += 1;
  ...
  for (let y = 0; y < canvas.height; y += stride) {
    ...
    for (let x = 0; x < canvas.width; x += stride) {
      ...
      if (x >= stride && y >= stride) {
        const diagonal = (y - stride) * canvas.width + x - stride;
        if (silhouette[current] && silhouette[diagonal]) {
          diagonalPairs += 1;
          if (Math.abs(luma[current] - luma[diagonal]) >= 0.12) {
            diagonalEdges += 1;
          }
        }
      }
    ...
    if (rowCount >= 8) rowMeans.push(rowSum / rowCount);
  }
  const rowMean = rowMeans.reduce((total, value) => total + value, 0)
    / Math.max(1, rowMeans.length);
  const rowCentered = rowMeans.map(value => value - rowMean);
  const rowEnergy = rowCentered.reduce(
    (total, value) => total + value * value,
    0,
  );
  let periodicBandScore = 0;
  let periodicBandPeriod = null;
  for (let period = 2; period <= Math.min(24, rowCentered.length / 3); period += 1) {
    let correlation = 0;
    let energy = 0;
    for (let index = period; index < rowCentered.length; index += 1) {
      correlation += rowCentered[index] * rowCentered[index - period];
      energy += (
        rowCentered[index] ** 2
        + rowCentered[index - period] ** 2
      ) / 2;
    }
  ...
  modelLuminance.sort((left, right) => left - right);
  const quantile = ratio => {
    if (!modelLuminance.length) return 0;
    const index = Math.min(
      modelLuminance.length - 1,
      Math.max(0, Math.round((modelLuminance.length - 1) * ratio)),
    );
    return modelLuminance[index];
  };
  return {
    width: canvas.width,
    ...
    darkRatio: darkCount / Math.max(1, nonBackgroundCount),
    ...
    diagonalStaircaseRatio: {
      algorithm:
        "diagonal in-silhouette neighbor luminance edges at delta >= 0.12",
      edgeCount: diagonalEdges,
      pairCount: diagonalPairs,
      ratio: diagonalEdges / Math.max(1, diagonalPairs),
    },
    periodicBandScore: {
      algorithm:
        "maximum absolute row-luminance autocorrelation over periods 4..48 pixels",
      rowEnergy,
      periodPixels: periodicBandPeriod,
      score: periodicBandScore,
    },
  };
}
...
async function applyScenario(diagnostics, scenario) {
  ...
  if (scenario.state === "split" || scenario.state === "split-explode") {
    setControl("sideSplit", 100);
  }
  if (scenario.state === "explode" || scenario.state === "split-explode") {
    setControl("explode", 100);
  }
  await diagnostics.waitForFrames(4);
}

function stage1Scenarios() {
  const scenarios = [];
  const requestedTheme = params.get("theme");
  const themes = requestedTheme ? [requestedTheme] : ["navy", "obsidian"];
  for (const theme of themes) {
    for (const view of ["front", "dialMechanism", "side", "movementBack"]) {
      for (const opacity of [1, 0.99, 0.75, 0.56, 0.55, 0.54, 0.53, 0.16, 0.08]) {
        scenarios.push({
          category: "shadow",
          id: [
            theme,
        ...
        if (opacity === 0.16 || opacity === 0.08) {
          for (const state of ["split", "explode"]) {
            scenarios.push({
              category: "shadow",
              id: [
                theme,
...
(async () => {
  ...
  const candidateState = diagnostics.getIssue2Phase3B1cState();
  if (!candidateState.enabled || candidateState.candidate !== candidate) {
    throw new Error(`Phase 3B.1c candidate did not initialize: ${candidate}`);
  }
  const report = {
    schemaVersion: 1,
    ...
    candidateState,
    shadow: diagnostics.getIssue2Phase3B1cShadowReport(),
  ...
  for (let index = 0; index < scenarios.length; index += 1) {
    ...
    status.textContent =
      `${candidate} ${index + 1}/${scenarios.length} ${scenario.id}`;
  ...
  report.finalShadow = diagnostics.getIssue2Phase3B1cShadowReport();
  const reportPath = evidencePath(
    "reports",
    [
      matrix === "motion" ? "motion" : "stage1",
      candidate,
      matrix === "stage1" && params.get("theme")
        ? params.get("theme")
        : null,
      viewportId,
