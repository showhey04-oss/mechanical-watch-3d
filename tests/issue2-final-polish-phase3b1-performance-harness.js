...
const output = document.getElementById("issue2Phase3B1PerformanceResult");
...
const waitFor = async (test, timeoutMs = 60_000) => {
  ...
  throw new Error("timed out waiting for Phase 3B.1 performance diagnostics");
};
...
async function postReport(report) {
  ...
  if (!response.ok) throw new Error(`performance report upload failed: ${response.status}`);
}
...
(async () => {
  ...
  const scenarios = [
    ...
    },
    {
      ...
      type: "front-idle",
      durationMs: 3_000,
      setup: diagnostics => diagnostics.setPhase3C1ExteriorGroupVisible(false),
    },
  ];
  const results = [];
  ...
  const report = {
    ...
    candidateState: diagnostics.getIssue2Phase3B1State(),
