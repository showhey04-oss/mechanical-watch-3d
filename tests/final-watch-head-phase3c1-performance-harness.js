...
const output = document.getElementById("phase3c1PerformanceResult");
const log = document.getElementById("phase3c1PerformanceLog");
...
function publish(result, status) {
  ...
  window.phase3c1PerformanceResult = result;
}
...
(async () => {
  ...
  for (const [type, durationMs] of [
    ...
    scenarios.push(await diagnostics.runPerformanceScenario({ type, durationMs }));
  }
