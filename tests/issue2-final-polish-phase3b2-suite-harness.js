...
const output = document.getElementById("issue2Phase3B2SuiteResult");
const log = document.getElementById("issue2Phase3B2SuiteLog");
...
const continuity =
  params.get("continuity") || "issue2-stable-depth-off";
...
async function publish(result, resultStatus) {
  ...
  if (resultStatus === "passed" || resultStatus === "failed") {
    ...
    const path =
      ...
      `reports/suite-${suite}-${rendering}-${continuity}-${viewport}.json`;
