...
const output = document.getElementById(
  "issue2Phase3B1cPerformanceResult",
);
...
const waitFor = async (test, timeoutMs = 60_000) => {
  ...
  throw new Error(
    "timed out waiting for Phase 3B.1c performance diagnostics",
  );
};
...
(async () => {
  ...
  const scenarios = [
    ...
    },
    {
      ...
      setup: async diagnostics => {
        for (const opacity of [
          1, 0.99, 0.75, 0.56, 0.55, 0.54, 0.53, 0.25, 0.16, 0.08,
        ]) {
    ...
    },
    {
      ...
      type: "front-idle",
      durationMs: 3_000,
      setup: diagnostics => diagnostics.pickProjectedPart("設定車2"),
    },
    {
      id: "split",
      type: "front-idle",
  ...
  for (const scenario of scenarios) {
    ...
    const before = diagnostics.getIssue2Phase3B1cShadowReport();
    await scenario.setup?.(diagnostics);
    await diagnostics.waitForFrames(12);
    const afterSetup = diagnostics.getIssue2Phase3B1cShadowReport();
    const result = await diagnostics.runPerformanceScenario({
      type: scenario.type,
      durationMs: scenario.durationMs,
    });
    const after = diagnostics.getIssue2Phase3B1cShadowReport();
    results.push({
      id: scenario.id,
      ...
      result,
      shadowRefresh: {
        before: before.shadow?.opacityShadowRefreshCount ?? null,
        afterSetup: afterSetup.shadow?.opacityShadowRefreshCount ?? null,
        after: after.shadow?.opacityShadowRefreshCount ?? null,
        setupDelta:
          (afterSetup.shadow?.opacityShadowRefreshCount ?? 0)
          - (before.shadow?.opacityShadowRefreshCount ?? 0),
        scenarioDelta:
          (after.shadow?.opacityShadowRefreshCount ?? 0)
          - (afterSetup.shadow?.opacityShadowRefreshCount ?? 0),
      },
    });
  }
  ...
  const report = {
    ...
    candidateState: diagnostics.getIssue2Phase3B1cState(),
    shadow: diagnostics.getIssue2Phase3B1cShadowReport(),
    thresholdsChanged: false,
    refreshGates: {
      idlePointerWheelOpacityExpected: 0,
      opacityUpdateShadowRefreshExpected: 0,
    },
    results,
  };
