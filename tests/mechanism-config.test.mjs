...
test("A.5 lighting keeps two keys and a subordinate camera-follow fill", async () => {
  ...
  assert.ok(source.includes("'studio-d1','studio-d2','studio-d2a','studio-d2b','studio-d2c1','studio-d2c2','studio-d2c3','studio-d3'"));
  assert.ok(source.includes("await import('./js/issue2-studio-lighting.js?version=phase2a2-1')"));
  assert.ok(source.includes("ISSUE2_ZOOM_STABLE_FOG_RANGE=Object.freeze({near:160,far:260})"));
  assert.ok(source.includes("ISSUE2_ZOOM_STABLE_STUDIO_CANDIDATES=new Set(['studio-d2a','studio-d2b','studio-d2c1','studio-d2c2','studio-d2c3'])"));
  assert.ok(source.includes("issue2ZoomStableStudioCandidate=ISSUE2_ZOOM_STABLE_STUDIO_CANDIDATES.has(issue2Candidate)"));
  assert.ok(source.includes("issue2D2CMidtoneCandidate=issue2Candidate.startsWith('studio-d2c')"));
  assert.ok(source.includes("issue2StartupGateSatisfied()"));
  assert.ok(source.includes("milestones.pmremReady&&milestones.environmentApplied&&milestones.candidateReady"));
  assert.ok(source.includes("issue2StudioRig?.rectLights.length===expectedRectLights"));
  assert.ok(source.includes("getIssue2InitialShadowMapState()"));
  assert.ok(source.includes("recordIssue2StartupStage('initial-shadow-ready',{mapSize:[shadowMap.width,shadowMap.height]})"));
  assert.ok(source.includes("nextDeadline=issue2StartupState.startedAt+sampleIndex*250"));
  assert.ok(source.includes("setTimeout(capture,Math.max(0,nextDeadline-performance.now()))"));
  assert.ok(source.indexOf("const issue2StartupTracePromise=startIssue2StartupTrace()") < source.indexOf("await import('./js/issue2-studio-lighting.js?version=phase2a2-1')"));
  assert.ok(source.includes("issue2StudioRig?.updateCameraOrientation?.(camera);updateCameraDataset()"));
  assert.ok(source.includes("issue2MaskBlackMaterial??=new THREE.MeshBasicMaterial({color:0x000000"));
  assert.ok(source.includes("node.material=included.has(node)?issue2MaskWhiteMaterial:issue2MaskBlackMaterial"));
  assert.ok(source.includes("method:'isolated-object-material-footprint'"));
  assert.ok(source.includes("visibleSurface:{...visibleSurface,occluded:visibleSurface.sampleCount===0}"));
  assert.ok(source.includes("watchSilhouette={...measureIssue2MaskedFramebuffer(frame,fullMask),method:'watch-root-silhouette-mask-pass'"));
  assert.ok(source.includes("meanLuminance:mean,averageLuminance:mean,medianLuminance:median,p10Luminance"));
  assert.ok(source.includes("regionRoots={dial:[dialRing,...Object.values(dialMarkers)],hands:[minuteAxis,hourAxis,secAxis],brassTrain:[barrelWheel,center,third],steelTrain:[fourth,escape,pallet],ruby:collectIssue2MaterialMeshes(MAT.ruby),plate:[mainPlate],outerBezel:[dialRing,plateEdgeTop,plateEdgeBottom]}"));
  assert.ok(source.includes("node.material=material;node.visible=visible"));
  assert.ok(source.includes("environmentIntensity=getIssue2EnvironmentIntensity()"));
  ...
  assert.ok(studioSource.includes('"studio-d2a":Object.freeze({rectLights:true,expectedRectLights:2,shadowCarrier:false,placementStrategy:"world-fixed",zoomStableFog:true,studioProfile:"base"})'));
  assert.ok(studioSource.includes('"studio-d2b":Object.freeze({rectLights:true,expectedRectLights:2,shadowCarrier:false,placementStrategy:"camera-orientation-fixed-radius",zoomStableFog:true,studioProfile:"base"})'));
  assert.ok(studioSource.includes("camera.getWorldPosition(new THREE.Vector3())"));
  assert.ok(studioSource.includes("quaternionRight.addScaledVector(view,-quaternionRight.dot(view))"));
  assert.ok(studioSource.includes("lastViewDirection:rig.orientationFollowing.lastView.toArray()"));
  assert.ok(studioSource.includes("scene.add(light)"));
  assert.equal(studioSource.includes("camera.add(light)"), false);
  assert.equal(studioSource.includes("desiredZoomDistance"), false);
  assert.equal(studioSource.includes("PointLight("), false);
  assert.ok(source.includes("requestedIssue2Candidate=initialPageParameters.get('issue2Candidate')||'baseline'"));
  assert.ok(source.includes("defaultRenderingUnchanged:!initialPageParameters.has('issue2Candidate')"));
  assert.ok(studioSource.includes('"studio-d2c1":Object.freeze({rectLights:true,expectedRectLights:2,shadowCarrier:false,placementStrategy:"world-fixed",zoomStableFog:true,studioProfile:"d2c1-midtone-environment"})'));
  assert.ok(studioSource.includes('"studio-d2c2":Object.freeze({rectLights:true,expectedRectLights:2,shadowCarrier:false,placementStrategy:"world-fixed",zoomStableFog:true,studioProfile:"d2c2-balanced-key-fill"})'));
  assert.ok(studioSource.includes('"studio-d2c3":Object.freeze({rectLights:true,expectedRectLights:3,shadowCarrier:false,placementStrategy:"world-fixed",zoomStableFog:true,studioProfile:"d2c3-lower-bounce"})'));
  assert.ok(studioSource.includes('ambientSurface:Object.freeze({kind:"room-and-floor",color:"#202020",roomColor:"#202020",floorColor:"#202020",floorRadiance:.7,roomRadius:70,floorSize:[96,96],floorPosition:[0,0,-24]})'));
  assert.ok(studioSource.includes('Object.freeze({name:"studioEnvFrontFlag",role:"front-edge-negative-fill",color:"#080808",size:[7,30],position:[-23,-25,3]})'));
  assert.ok(studioSource.includes('Object.freeze({name:"studioEnvBackFlag",role:"back-edge-negative-fill",color:"#080808",size:[8,28],position:[24,22,-4]})'));
  assert.ok(studioSource.includes('Object.freeze({name:"studioRectKey",role:"front-key",color:"#ffffff",intensity:.85,size:[30,20],position:[15,-28,18]})'));
  assert.ok(studioSource.includes('Object.freeze({name:"studioRectFill",role:"back-fill",color:"#ffffff",intensity:.455,size:[32.2,25.3],position:[-18,27,11]})'));
  assert.ok(studioSource.includes('Object.freeze({name:"studioRectLowerBounce",role:"lower-front-bounce",color:"#ffffff",intensity:.085,size:[38,24],position:[0,-22,-26]})'));
  assert.ok(studioSource.includes('"d2c1-midtone-environment":Object.freeze({environment:D2C_MIDTONE_ENVIRONMENT,rectLights:ISSUE2_STUDIO_LAYOUT.rectLights})'));
  assert.ok(studioSource.includes('"d2c2-balanced-key-fill":Object.freeze({environment:D2C_MIDTONE_ENVIRONMENT,rectLights:D2C_BALANCED_RECT_LIGHTS})'));
  assert.ok(studioSource.includes('"d2c3-lower-bounce":Object.freeze({environment:D2C_MIDTONE_ENVIRONMENT,rectLights:Object.freeze([...D2C_BALANCED_RECT_LIGHTS,D2C_LOWER_BOUNCE])})'));
  assert.ok(studioSource.includes("createStudioEnvironmentScene(studioProfile.environment)"));
  assert.ok(studioSource.includes("for(const definition of studioProfile.rectLights)"));
  assert.ok(studioSource.includes("directLightBalance:"));
  assert.equal(studioSource.includes("AmbientLight("), false);
});
