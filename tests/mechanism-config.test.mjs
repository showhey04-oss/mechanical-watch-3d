...
test("A.5 lighting keeps two keys and a subordinate camera-follow fill", async () => {
  ...
  assert.ok(source.includes("'studio-d1','studio-d2','studio-d2a','studio-d2b','studio-d3'"));
  assert.ok(source.includes("await import('./js/issue2-studio-lighting.js?version=phase2a1-3')"));
  assert.ok(source.includes("ISSUE2_ZOOM_STABLE_FOG_RANGE=Object.freeze({near:160,far:260})"));
  assert.ok(source.includes("issue2ZoomStableStudioCandidate=issue2Candidate==='studio-d2a'||issue2Candidate==='studio-d2b'"));
  ...
  assert.ok(source.indexOf("const issue2StartupTracePromise=startIssue2StartupTrace()") < source.indexOf("await import('./js/issue2-studio-lighting.js?version=phase2a1-3')"));
  ...
  assert.ok(source.includes("method:'isolated-object-material-footprint',visibleSurface"));
  ...
  assert.ok(studioSource.includes('"studio-d2a":Object.freeze({rectLights:true,shadowCarrier:false,placementStrategy:"world-fixed",zoomStableFog:true})'));
  assert.ok(studioSource.includes('"studio-d2b":Object.freeze({rectLights:true,shadowCarrier:false,placementStrategy:"camera-orientation-fixed-radius",zoomStableFog:true})'));
