...
- 公開方式：GitHub Pages
- 現在の改修フェーズ：最終外装統合 — Phase 3A 外装インターフェース監査・候補設計
...
寸法・比率調整は、PR #10のS86採用とPR #11のPhase 2C Y基準面・厚さレイヤー監査をもって完了扱いとする。Phase 3Aではムーブメント外径36.6、S86表示寸法、Phase 2Cの3包絡を保護アンカーとし、外装接続条件とE-COMPACT／E-BALANCED／E-EDUCATIONALの3候補だけを監査資料上で比較する。外装Geometryは通常表示へ追加せず、推奨候補E-BALANCEDも`RECOMMENDED_NOT_ADOPTED`として人間承認まで既定化しない。ETA 4.50 mmの基準面対応は未解決であり、最終外装実装時に表示開口と全体比率を再確認する。
