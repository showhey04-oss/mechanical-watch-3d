...
寸法・比率調整は、PR #10のS86採用とPR #11のPhase 2C Y基準面・厚さレイヤー監査をもって完了扱いとする。Phase 3Aで実装候補として承認されたE-BALANCEDを、Phase 3B.1では`?exterior=balanced`の明示query時だけThree.js Geometryとして生成する。状態は`IMPLEMENTATION_CANDIDATE_NOT_DEFAULT`であり、通常URLには外装Object3Dを追加せず、第2候補の人間確認前に既定採用しない。今回の対象はケース胴、ベゼル、風防、rehaut、物理文字板blank、裏蓋リング・観察窓、ムーブメント保持リング、ケースチューブ、局所接続候補である。ラグ／ストラップはPhase 3B.2へ分離する。りゅうず位置1、指掛かり、pull／push、位置2、カメラ、外装選択、透過100%／16%、時計機能、作動音、PC／物理iPhone性能は第1候補の人間確認で合格済みで、透過50%は判断保留である。ETA 4.50 mmの基準面対応も未解決であり、表示開口と全体比率は外装統合レビューで再確認する。

Phase 3B.1第2候補ではケース胴Y範囲を-2.860～4.635、ケース胴軸方向厚を7.495、風防／裏蓋リング厚を各0.600、外装総厚を8.695へ縮小した。ベゼルは表示開口29.800、背面外径38.800、正面外径37.600の単一閉合テーパープロファイルとし、保持リングは外径37.650／内径36.750／Y=4.035～4.485で追加した。実りゅうずMesh由来の採用逃げ0.310872は上限0.330以内、位置1実gap 0.030088、最小壁厚0.589128である。内周半径18.900、りゅうず・巻真・チューブ、S86、Phase 2C包絡、機構、描画、UI、作動音は変更していない。
