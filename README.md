...
Phase 3C.1は、人間承認済みPhase 3B.2 Head `98d83781aa7aa001836a0d57f1ad6e3d058a15c4`へ積み上げる別Draftである。初回、第2候補、第3候補は人間非承認で、状態は`HUMAN_REVIEW_FAILED_PHASE3C1_THIRD_REVISION_REQUIRED`。第4候補は`?exterior=balanced&watchHead=phase3c1`時だけ、白系アイボリー文字板`#F2EDE5`／小秒面`#F5F1EA`、教育表示用の安定silver`#E7EAED`、大型faceted bar、半径14.200の同径60分目盛、視覚径8.500の小秒recess、外径7.120の細身open-heart rim、文字板をぼかさない非屈折近似domeを生成する。実テンプ投影[7.700,1.800]、開口径6.600、中央軸受を保持した物理地板窓、S86、機構を維持し、候補専用の学習タブ「外装」で29部品を制御しながら既存FRONT／CORE／BACK／PLATEの表裏分離・分解transformへ統合した。通常URLはPhase 3B.2とpixel exactで、A.6性能は絶対・差分基準に合格する。Desktop総合では白系文字板によりA.5前後面明度差だけが未達、音声integrationは同一環境のBaseと候補双方でtimeoutしたため、閾値・照明を変更せず記録している。第4候補はPC／物理iPhone再確認待ち、既定未採用である。表裏分離／断面クリップのUX整理は`UI_SIMPLIFICATION_REVIEW_AFTER_PHASE3C2_AND_ISSUE2`へ申し送る。`PHYSICAL_IPHONE_MILD_WARMING_AFTER_15_MIN`は非ブロッキング観察事項として最終15分連続確認へ残し、黒革、巻込み部、実用長、穴列、定革・遊革、尾錠枠・つく棒・取付バーはPhase 3C.2の必須後続工程とする。
