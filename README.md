...
- main公開基準：v3.13.0（PR #4 モバイルオーバーレイ・HUD整理）
- Draft候補：PR #6 / v3.14.0（機構同期作動音 Phase 1、未マージ）
- 基準形式：ETA 6498-1級の大型手巻きムーブメント
- 公開方式：GitHub Pages
- 現在の重点：Draft PR #6で、脱進機・巻上げ・逆転空転・りゅうず位置切替へ同期する初期OFFの合成作動音を検証
...
- 既存の機構角・ビート位相を読み取って発音する、初期OFF・遅延初期化の機構同期作動音
...
2. りゅうず位置1／位置2のユーザー操作完了時だけpull／push音を1回発音し、初期化・復元・診断では無音を維持
3. Web Audioを初期OFF・ユーザー操作後の遅延初期化とし、音量、可視状態、読み込み失敗を独立管理
...
設計と証跡は `docs/MECHANICAL_SOUND_SYSTEM.md` と `docs/evidence/mechanical-operation-sounds/README.md` を参照してください。実機iPhoneでの音量・音色・遅延・二重発音確認は人手の最終受入項目です。
