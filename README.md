...
寸法・比率調整は、PR #10のS86採用とPR #11のPhase 2C Y基準面・厚さレイヤー監査をもって完了扱いとする。Phase 3Aで実装候補として承認されたE-BALANCEDを、Phase 3B.1では`?exterior=balanced`の明示query時だけThree.js Geometryとして生成する。状態は`IMPLEMENTATION_CANDIDATE_NOT_DEFAULT`であり、通常URLには外装Object3Dを追加せず、第3候補の人間確認前に既定採用しない。今回の対象はケース胴、ベゼル、風防、rehaut、物理文字板blank、裏蓋リング・観察窓、ムーブメント保持リング、ケースチューブ、局所接続候補である。ラグ／ストラップはPhase 3B.2へ分離する。外装総厚8.695の方向性、りゅうず位置1／2、指掛かり、pull／push、保持リング方針、透過16%、回転・ズーム、時計機能、作動音、物理iPhone操作性は人間確認合格済みで、透過50%と増強した3か所のテーパーは判断保留である。ETA 4.50 mmの基準面対応も未解決であり、表示開口と全体比率は外装統合レビューで再確認する。

Phase 3B.1第3候補は総厚8.695、ケース胴厚7.495、風防／裏蓋リング厚各0.600、表示開口29.800、保持リング外径37.650／内径36.750／Y=4.035～4.485を維持したまま、ケース胴端径38.900、ベゼル正面外径37.000（内周正面Y=-3.240／外縁正面Y=-2.880）、裏蓋後面外径37.800へテーパーだけを増強した。実りゅうずMesh由来の必要逃げ0.298836、採用逃げ0.319108、上限差0.010892、位置1実gap 0.030026、最小壁厚0.550000である。内周半径18.900、りゅうず・巻真・チューブ、S86、Phase 2C包絡、機構、描画、UI、作動音は変更していない。
