...
寸法・比率調整は、PR #10のS86採用とPR #11のPhase 2C Y基準面・厚さレイヤー監査をもって完了扱いとする。Phase 3Aで実装候補として承認されたE-BALANCEDを、Phase 3B.1では`?exterior=balanced`の明示query時だけThree.js Geometryとして生成する。状態は`IMPLEMENTATION_CANDIDATE_NOT_DEFAULT`であり、通常URLには外装Object3Dを追加せず、実画面の人間確認前に既定採用しない。今回の対象はケース胴、ベゼル、風防、rehaut、物理文字板blank、裏蓋リング・観察窓、ケースチューブ、局所接続候補である。ラグ／ストラップはPhase 3B.2へ分離し、指掛かりとpull／push操作性は`UNVERIFIED`のまま維持する。ETA 4.50 mmの基準面対応も未解決であり、表示開口と全体比率は外装統合レビューで再確認する。

Phase 3B.1追補ではケース胴を前後径39.000・中央最大径39.600の軸方向プロファイルへ変更し、実りゅうずMeshから算出した局所逃げだけを外周へ滑らかに適用する。採用逃げ0.309461は上限0.330以内、位置1実gap 0.030084、最小壁厚0.590539である。内周半径18.900、りゅうず・巻真・チューブ、S86、機構、描画、UI、作動音は変更していない。
